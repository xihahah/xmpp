        ...
        if (cantStartShot) {
            html2canvas(document.body, {
                onrendered:function(canvas) {
                    ...
                    var startY = 0;
                    var isShot = false;
                    var layerName = "name" + Math.random();
                    //图层名字，每次删除上一图层
                    var canvasWrapperId = "canvasWrapperId";
                    //最后确定框图
                    ...
                    css += ".result {position: fixed;z-index: 4;cursor: pointer;}.result a {padding: 5px 15px;border: 1px solid;border-radius: 12px;margin-right: 5px;font-size: 12px; color:white}</style>";
                    $("#" + canvasId).remove();
                    $("head").append(css);
                    $("body").css("cursor", "crosshair").append("<canvas id=" + canvasId + " width=" + canvas.width + " height=" + canvas.height + "></canvas>");
                    //添加图层用来画选择框
                    $(copyDomCanvas).addClass("copy-body");
                    //html转化来的canvas图片作为背景
